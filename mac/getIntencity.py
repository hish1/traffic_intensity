...
def make_json(array, file):
    with open(file+"getIntencity.json", "w") as write_file:
...
def read_json(file):
    array = 0
    with open(file+"getIntencity.json", "r") as f:
        array = json.load(f)
...
def getTrafficInPointIntencityOptimized(traceMatrix,  minLon, stepLon, maxLon, minLat, stepLat, maxLat, file):
    f = [True, False]
    start = time.time()
    ...
    if f[1]:
        ...
        print(f'Intencity: {time.time()-start}')
    ...
    if f[1]:
        ...
        print(f'Intencity with weight: {time.time()-start}')
    ...
    if f[1]:
        ...
        with open(file+"getIntencitylocGreen.json", "w") as write_file:
            json.dump(locGreen, write_file)
        with open(file+"getIntencitylocOrange.json", "w") as write_file:
            json.dump(locOrange, write_file)
        with open(file+"getIntencitylocRed.json", "w") as write_file:
            json.dump(locRed, write_file)

        print(f'Intencity colored: {time.time()-start}')
        exit()
