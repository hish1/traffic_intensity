...
def make_json(array, file):
    with open(file+"getMetricSize.json", "w") as write_file:
...
def read_json(file):
    array = 0
    with open(file+"getMetricSize.json", "r") as f:
        array = json.load(f)
...
def getTrafficInPointIntencityOptimizedMetricSize(traceMatrix,  minLon, stepLon, maxLon, minLat, stepLat, maxLat, sizeMatrix, file):
    f = [True, False]
    start = time.time()
    ...
    if f[0]:
        ...
        for i in range(1, mn):
            ...
            currLat =  traceMatrix[i][1] + moveLat
            currLon =  traceMatrix[i][2] + moveLon
            ...
            if currLat >= minLat and currLat <= maxLat and currLon >= minLon and currLon <= maxLon:
                ...
                if isInCellMatrixFlag == 0:
                    for k in range(1, mnSize):
                        if traceMatrix[i][0] == sizeMatrix[k][0]:
                            shipSize = sizeMatrix[k][1]
                            break

                    # per 100 meters
                    trafficIntensity[iLat][iLon] += floor(shipSize / 100)  + 1
                    if isInCellMatrixRowNumber<len(isInCellMatrix)-1:
    ...
    if f[0]:
        ...
        print(f'Metric Size with weight: {time.time()-start}')
    ...
    if f[0]:
        ...
        for i in range(len(array)):
            for j in range(len(array[i])):
                if array[i][j][2]>50000:
                    locRed.append([array[i][j][0], array[i][j][1]])
                elif array[i][j][2]>8000:
        ...
        with open(file+"getMetricSizelocGreen.json", "w") as write_file:
            json.dump(locGreen, write_file)
        with open(file+"getMetricSizelocOrange.json", "w") as write_file:
            json.dump(locOrange, write_file)
        with open(file+"getMetricSizelocRed.json", "w") as write_file:
            json.dump(locRed, write_file)

        print(f'Metric Size colored: {time.time()-start}')
        exit()
